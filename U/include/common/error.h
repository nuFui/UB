...
#define ERROR_BODY(file, function, line)                                                                           \
  fprintf(stderr, "%s[%s]: file: %s, function: %s, line: %d, message: ", RED, __FUNCTION__, file, function, line); \
  vfprintf(stderr, fmt, details);                                                                                  \
  fprintf(stderr, ".%s\n", RESET);                                                                                 \
  exit(EXIT_FAILURE);

extern void error_raise(void (*error_func)(error_pos_t *, const char *, va_list), error_pos_t *pos, const char *fmt, ...);
